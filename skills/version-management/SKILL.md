...
description: 独立通用 Skill：管理前端项目全生命周期。当任何 Skill 产出 .html/.jsx/.tsx/.vue 入口文件时自动触发；也响应用户的版本相关操作（查看历史、恢复版本、切换项目等）。
---
...
→ 产出 .html / .jsx / .tsx / .vue → 文件落入 我的项目/{项目名}/
→ quality-gate.md 通过
...
1. **写入前检查**：在调用方 Skill 生成文件时，检查目标路径
2. **若文件已在正确位置**（`我的项目/{项目名}/` 下）→ 继续后续流程
3. **若文件不在正确位置**（如 workspace root、`~/Desktop`、聊天中的代码块）→ **必须移动或重写**到 `我的项目/{项目名}/`
4. **项目名生成规则**：根据用户意图、内容主题自动生成语义化中文或英文项目名（如 `北京旅游网站`、`企业客户管理后台`）

```bash
# 强制路径检查示例
if [ ! -f "我的项目/{项目名}/index.html" ]; then
  # 文件不在正确位置，拒绝 commit，必须先移动到正确路径
  echo "ERROR: 入口文件必须在 我的项目/{项目名}/ 下"
fi
```
...
1. 根据用户意图生成项目名称
2. 若「我的项目」不存在：`mkdir -p 我的项目/`
3. 创建项目目录与 assets：`mkdir -p 我的项目/{项目名}/assets`
...
| 用户行为 | 执行动作 |
|--------|--------|
| 「做一个 XX 网站」 | 生成文件 → §1 创建（git init + V1）→ `send_file` |
| 「做一个产品原型（prototype）」 | 生成 `prototype.html` + `flow.html` → 同版 commit（两文件都进 Git）→ 按 §3.4.3 连续两次 `send_file` |
